...
> 🖥️ **Ambiente:** Arch Linux · VS Code (extensão HashiCorp Terraform) · `terraform` CLI · `azure-cli`
...
graph LR
    ...
    RG --> VN[🌐 VNet / Subnet]
    RG --> NSG[🛡️ NSG - SSH restrito por IP]
...
**Fix — descobrir as regiões liberadas direto na fonte:**
```bash
...
**Fix:** trocar apenas o `size`, tentando outra família (B2s, D2s_v3 etc). Nunca trocar a `location` só por causa disso — força destroy+create de RG/VNet/NIC à toa (ver item 5).
</details>
...
**Regra prática:** trocar região = sempre esperar um ciclo completo de destroy antes de aplicar de novo. Não empilhar troca de `size` e `location` no mesmo apply.
</details>
...
**Causa:** a Azure não libera nenhuma porta de entrada por padrão. Sem um Network Security Group associado à placa de rede, o tráfego externo (incluindo a porta 22) é descartado silenciosamente — o terminal fica esperando indefinidamente, sem erro explícito.
...
- 🧩 **Sem hardcode de infraestrutura pessoal** — usuário admin, caminho da chave SSH e IP autorizado ficam em variáveis, não fixos no `main.tf` — facilita reuso do template sem expor dados específicos do autor original.
- 🔑 **Chave privada SSH protegida por passphrase** (`ssh-keygen -p -f ~/.ssh/id_rsa`) — mitiga o risco de uso indevido da chave em caso de comprometimento do disco/máquina.
...
- [x] Validar acesso SSH na VM — confirmado, funcionando de ponta a ponta ✅
